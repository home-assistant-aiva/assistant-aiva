...
En Windows RC4, el instalador `AIVA-Collector-Setup-v0.2.7-desktop-rc4.exe` instala Desktop, CLI y Background mediante una distribución `onedir` con dependencias compartidas. La tarea programada usa exclusivamente `aiva-collector-background.exe run-auto --config "%ProgramData%\AIVA\Collector\config.windows.json"`, conserva ProgramData y no pasa tokens por argumentos.
